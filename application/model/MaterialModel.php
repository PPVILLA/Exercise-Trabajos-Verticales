...
class MaterialModel
{
    ...
    public static function getMaterialsSuggestedPaginatedOrderBy($suggestion, $start, $itemsToShow, $orderBy)
    {
        $database = DatabaseFactory::getFactory()->getConnection();

        $sql = "SELECT material_id, material_name, material_price, material_weight, material_dimension_high, material_dimension_width,
                       material_dimension_profound, material_provider_id, material_has_photoMaterial, material_description
                FROM materials WHERE material_name LIKE '%$suggestion%' OR material_description LIKE '%$suggestion%' ORDER BY $orderBy LIMIT $start, $itemsToShow";
        $query = $database->prepare($sql);
